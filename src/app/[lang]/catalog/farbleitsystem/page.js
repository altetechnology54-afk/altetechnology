...
export default async function FarbleitsystemPage({ params }) {
    ...
    return (
        <main className="min-h-screen bg-white font-sans text-slate-900 pb-24 px-3 sm:px-6 md:px-12 max-w-[1200px] mx-auto">
            {/* Header */}
            <div className="pt-6 md:pt-12 mb-6 md:mb-12 border-b border-slate-100 pb-4">
                <h1 className="text-2xl sm:text-3xl md:text-5xl font-light text-slate-800 tracking-tight mb-3">
                    Farbleitsystem
                </h1>
                <h2 className="text-lg md:text-2xl font-bold text-[#1A3694]">
                    1. Implantate
                </h2>
                <h3 className="text-sm md:text-xl font-bold text-[#1A3694] italic">
            ...
            <section className="mb-16 md:mb-20">
                ...
                <div className="overflow-x-auto pb-4 -mx-3 px-3">
                    <div className="min-w-[500px] md:min-w-0 grid grid-cols-[110px_1fr] md:grid-cols-[160px_1fr] gap-3 md:gap-8 items-center mb-8">
                        {/* Left Labels */}
                        <div className="space-y-12 md:space-y-16 text-xs md:text-sm font-bold text-slate-800">
                            <div>
                                <p>Implantat</p>
                                <p>Durchmesser</p>
                                <p className="text-[#1A3694] font-normal italic text-[11px] md:text-xs">Implant</p>
                                <p className="text-[#1A3694] font-normal italic text-[11px] md:text-xs">Diameter</p>
                            </div>
                            <div className="pt-16 sm:pt-24 md:pt-36">
                                <p>Farbcode</p>
                            </div>
                        ...
                        <div className="grid grid-cols-5 gap-2 md:gap-4 items-center">
                            {diameters.map((d, idx) => (
                                <div key={idx} className="flex flex-col items-center space-y-3 md:space-y-6">
                                    {/* Top Diameter Circle */}
                                    <div
                                        className={`w-12 h-12 md:w-20 md:h-20 rounded-full flex items-center justify-center font-black text-[9px] md:text-sm shadow-md ${d.border || ''}`}
                                    ...
                                    {/* Farbcode Box */}
                                    <div
                                        className={`w-full py-1 md:py-2 text-center text-[10px] md:text-sm font-black uppercase rounded shadow-sm ${d.border || ''}`}
                                        style={{ backgroundColor: d.hex, color: d.textColor }}
                                    >
                                        {d.color}
                                    </div>
                                    ...
                                    {/* Farbmarkierung Dot */}
                                    <div
                                        className={`w-7 h-7 md:w-12 md:h-12 rounded-full shadow-inner ${d.border || ''}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mt-12 md:mt-16 pt-8 border-t border-slate-100">
                    {/* Tube in Tube */}
                    <div className="flex flex-col items-center bg-slate-50/50 rounded-xl p-3 border border-slate-100">
                        <div className="w-full bg-[#DCE6F2] py-2 px-3 text-center font-bold text-xs md:text-sm text-slate-800 rounded-lg mb-4">
                            Tube in Tube
                        </div>
            ...
            <section className="mt-14 md:mt-20 pt-8 md:pt-12 border-t-2 border-slate-200">
                <h2 className="text-lg md:text-2xl font-bold text-[#1A3694] mb-6 md:mb-8">
                    2. Farbcodierung Bohrer / <span className="italic font-normal">Color Code Drills</span>
                </h2>

                <div className="max-w-4xl mx-auto space-y-8 md:space-y-12">
                    {/* 1. Row: Boxes & Original Size Drills (Scrollable on small screens) */}
                    <div className="overflow-x-auto pb-4 -mx-3 px-3">
                        <div className="min-w-[500px] md:min-w-0 grid grid-cols-[110px_1fr] md:grid-cols-[150px_1fr] gap-3 md:gap-4 items-center">
                            <div className="text-xs md:text-sm font-bold text-slate-800">
                                <p>Original Größe</p>
                                <p className="text-[#1A3694] font-normal italic text-[11px] md:text-xs">Original Valum</p>
                            </div>
                    ...
                    {/* 2. Row: Countersink Drills */}
                    <div className="w-full flex justify-center py-2 md:py-4">
                        <img
                            src="/images/farbleitsystem/b6.png"
                            alt="Countersink Drills"
                            className="max-w-[240px] sm:max-w-[280px] md:max-w-[340px] w-full h-auto object-contain"
                        />
                    </div>

                    {/* 3. Row: Drills with Length Markers */}
