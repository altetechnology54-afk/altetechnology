                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl md:rounded-[60px] p-4 md:p-16 shadow-xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 max-w-4xl space-y-6 md:space-y-12">
                        ...
                        {sectionData.articles?.length > 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 pt-6 md:pt-12">
                                {sectionData.articles.map((article, i) => (
                                    <div key={i} className="bg-white p-3 md:p-6 rounded-xl md:rounded-3xl border border-slate-100 shadow-sm flex items-center gap-3 md:gap-6">
                                        {article.image && (
                                            <div className="w-16 h-16 md:w-24 md:h-24 flex-shrink-0 bg-slate-50 rounded-lg md:rounded-2xl p-1 md:p-2">
                                                <img src={article.image} alt={article.artNr} className="w-full h-full object-contain" />
                                            </div>
                                        )}
                                        <div>
                                            <h4 className="font-black text-slate-800 uppercase tracking-tight text-xs md:text-base">{article.artNr}</h4>
                                            <p className="text-[10px] md:text-sm text-slate-500 italic">{getT(article.description)}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
